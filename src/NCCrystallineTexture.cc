  ...
  namespace {
                          ...
                          double d_hkl, double R, double wl )
    {
      ...
      if ( sin_theta >= -1. && sin_theta <= 1. ) {
        ...
        //  double x1mx = x * std::sqrt(1.0 - NC::ncsquare(x));
        //  P_hkl += 1. / std::sqrt( std::pow( 4 * a * NC::ncsquare(x1mx) + 2 * b * x1mx + c, 3 ) * (1.0 - NC::ncsquare(x)) );
        //}
        //double x1 = -1.+epsilon * std::sqrt(1.0 - NC::ncsquare(-1.+epsilon));
        //P_hkl -= 0.5 / std::sqrt( std::pow( 4 * a * NC::ncsquare(x1) + 2 * b * x1 + c, 3 ) * (1.0 - NC::ncsquare(-1.+epsilon)) );
        //P_hkl -= 0.5 / std::sqrt( std::pow( 4 * a * NC::ncsquare(x1) - 2 * b * x1 + c, 3 ) * (1.0 - NC::ncsquare(-1.+epsilon)) );
